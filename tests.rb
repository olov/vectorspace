...
class TestVec < Test::Unit::TestCase
  ...
  def test_add
    v1 = Vec[1, 2, 3]
    v2 = Vec[4, 5, 6]
    assert_equal(v1 + v2, Vec[5, 7, 9])
    assert_equal(v1 + v2, v1.add(v2))
    assert_equal(v1 + 2, Vec[3, 4, 5])
    assert_equal(v1 + 2, v1.add(2))
  end
  def test_sub
    v1 = Vec[4, 6, 8]
    v2 = Vec[1, 2, 3]
    assert_equal(v1 - v2, Vec[3, 4, 5])
    assert_equal(v1 - v2, v1.sub(v2))
    assert_equal(v1 - 2, Vec[2, 4, 6])
    assert_equal(v1 - 2, v1.sub(2))
  end
  def test_mul
    v1 = Vec[4, 6, 8]
    v2 = Vec[1, 2, 3]
    assert_equal(v1 * v2, Vec[4, 12, 24])
    assert_equal(v1 * v2, v1.mul(v2))
    assert_equal(v1 * 3, Vec[12, 18, 24])
    assert_equal(v1 * 3, v1.mul(3))
  end
  def test_div
    v1 = Vec[4, 6, 15]
    v2 = Vec[1, 2, 3]
    assert_equal(v1 / v2, Vec[4, 3, 5])
    assert_equal(v1 / v2, v1.div(v2))
    assert_equal(v1 / 3.0, Vec[4/3.0, 2, 5])
  ...
  def test_coerce
    v1 = Vec[-1, 1, 3]
    assert_equal(Vec[1, 3, 5], 2 + v1)
    assert_equal(Vec[3, 1, -1], 2 - v1)
    assert_equal(Vec[-2, 2, 2.0/3], 2.0 / v1)
    assert_equal(Vec[-2, 2, 6], 2 * v1)
  end
end
